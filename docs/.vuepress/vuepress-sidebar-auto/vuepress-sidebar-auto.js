...
/**
 * 递归获取分组信息并排序*/
function getGroupChildren(path, ele, root, ignoreDir) {
  if (ignoreDir.includes(ele)) {
    return;
  }
  ...
  pa.forEach(function (item, index) {
    ...
    if (info.isDirectory()) {
      ...
      getGroupChildren(path + "/" + ele, item, children, ignoreDir);
...
/**
 * 初始化*/
function getChildren(path, ele, options) {
  const { ignoreDir = [] } = options;
  var root = [];
  getGroupChildren(path, ele, root, ignoreDir);
  return root;
}
